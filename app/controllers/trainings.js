...
const createStudentTraining = async (req, res) => {
  try {
    ...
    res.status(500).send({
      message:
        err.message ||
        'Some error occurred while creating the StudentTraining.',
    });
  }
};
  ...
  StudentTraining.find()
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          'Some error occurred while retrieving student training data.',
      });
    });
};

const pick = (body) => _.pick(body, ['trainingType']);

const updateStudentTraining = (req, res) => {
  const studentId = req.params.studentId;
  StudentTraining.findOneAndUpdate({ studentId }, pick(req.body), {
    new: true,
  })
    .then((id) => {
      if (!id) {
