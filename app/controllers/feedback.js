...
.then( studentData=> {
    ...
    res.status(500).send({
        message: err.message || "Some error occurred while retrieving student feedback."
    });
});
};
...
// Find note and update it with the request body
  const updateStudentFeedback=(req,res)=>{
    ...
    console.log(pick);
    const studentId = req.params.objectId;
  StudentFeedback.findOneAndUpdate({studentId},
      pick(req.body),
      {new: true}
      )
.then(id => {
...
const deleteStudentFeedback = (req, res) => {
    ...
    .then(id => {
        ...
        return res.status(500).send({
            message: "Could not delete studentfeedback with id " + req.params.objectId
        });
    });
};
